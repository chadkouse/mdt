...
<?php
	...
	function LogUserIn( $collar,$password,$sessionid )
	{
		...
		if($q->num_rows > 0){
			if(password_verify($password, $a['password'])){
				...
				$q = $con->query("INSERT INTO mdt_logs VALUES (NULL, '{$a['userid']}', 'Has created a new session " . $_SERVER['REMOTE_ADDR'] . ".', 'Session', " . time() .", 1)");

				if($_SERVER['REMOTE_ADDR'] != $a['last_ip']){
					$con->query("INSERT INTO mdt_logs VALUES (NULL, '{$a['userid']}', 'Has logged in from another IP, possible account breach. Old IP: " . $a['last_ip'] . " | New IP: " . $_SERVER['REMOTE_ADDR'] . ".', 'Security', " . time() .", 1)");
				}
				return true;
			}else{
				$q = $con->query("INSERT INTO mdt_logs VALUES (NULL, '{$a['userid']}', 'Failed to sign in from the IP " . $_SERVER['REMOTE_ADDR'] . ".', 'Security', " . time() .", 1)");
				return false;
			}
		}else{
			return false;
	...
	function getGroupInfo($groupid)
	{
		...
		$q = $con->query("SELECT * FROM mdt_usergroups WHERE id = '{$groupid}'");
		$a = mysqli_fetch_assoc($q);
	...
	function searchCivs($firstname,$lastname,$dob)
	{
		global $con;
		global $streets;

		$q = $con->query("SELECT id FROM characters WHERE firstname = '{$firstname}' AND lastname = '{$lastname}' AND dateofbirth = '{$dob}'");
...
//			return $a['civid'];
	...
	function searchVehicle($vrm)
	{
		...
		$q = $con->query("SELECT plate FROM owned_vehicles WHERE plate = '{$vrm}'");
...
//			return $a['vehicleid'];
	...
	function getVehiclesForCiv($civid)
	{
		...
		$q = $con->query("SELECT 'plate' as vehicle, plate as vrm, plate as vehicleid FROM owned_vehicles WHERE owner = '{$civid}'");
		$vehicles = array();
	...
	function getVehicleInfo($vehicleid)
	{
        ...
        $q = $con->query("SELECT id, tickets, arrests FROM characters WHERE identifier = '{$a["owner"]}'");
        ...
        if (!empty($b)) {
            ...
            $markers = "";
            if ($b["arrests"] > 0) {
                $markers = $b["arrests"] . " arrest" . ($b["arrests"] > 1 ? "s" : "");
            }
            if ($b["tickets"] > 0) {
                if (!empty($markers)) {
                    $markers .= " | ";
                }
                $markers .= $b["tickets"] . " ticket" . ($b["tickets"] > 1 ? "s" : "");
            }
            $a["markers"] = $markers;
        }
    ...
    function getTicketsForCiv($civid)
    {
        global $con;

        $q = $con->query("SELECT issuer as 'officer', issued as 'criminal', reason, amount FROM tickets WHERE linkcid = '{$civid}'");
        $records = array();
    ...
    function getArrestsForCiv($civid)
    {
        global $con;

        $q = $con->query("SELECT cop as 'officer', perp as 'criminal', reason, time FROM arrests WHERE cid = '{$civid}'");
        $records = array();
	...
	function getCivInfo($civid)
	{
		...
		$q = $con->query("SELECT identifier, firstname, lastname, dateofbirth, arrests, tickets FROM characters WHERE id = '{$civid}'");
		$a = mysqli_fetch_assoc($q);
		if (!empty($a)) {
		    $markers = "";
		    if ($a["arrests"] > 0) {
		        $markers = $a["arrests"] . " arrest" . ($a["arrests"] > 1 ? "s" : "");
            }
            if ($a["tickets"] > 0) {
                if (!empty($markers)) {
                    $markers .= " | ";
                }
                $markers .= $a["tickets"] . " ticket" . ($a["tickets"] > 1 ? "s" : "");
            }
		    return array(
		        "cid" => $civid,
		        "civid" => $a['identifier'],
	...
	function getVehicleOwner($ownerid)
	{
		...
		$q = $con->query("SELECT name, firstname, lastname FROM users WHERE identifier = '{$ownerid}'");
		$a = mysqli_fetch_assoc($q);
		if (!empty($a)) {
		    return array("name"=>$a["firstname"] . " " . $a["lastname"] . " (" . $a["name"] . ")", "civid" => $ownerid);
        }
        return array("name"=>"Unknown", "civid" => $ownerid);
	}
	...
	function groupHasPerm($groupid,$perm)
	{
        ...
        $grpq = $con->query("SELECT * FROM mdt_usergroups WHERE id = '{$groupid}'");
	...
	function haveGeneralPerm($userid, $perm)
	{
        ...
        if (count($user)) {
            ...
            foreach ($groups as $group) {
                $grpq = $con->query("SELECT * FROM mdt_usergroups WHERE id = '{$group}'");
                $grp = mysqli_fetch_assoc($grpq);
	...
	function newLogEntry($user, $content, $cat)
	{
		...
		$q = $con->query("INSERT INTO mdt_logs VALUES (NULL, '{$user}', '{$content}', '{$cat}', '{$date}', 1)");
	...
	function getLogs($type)
	{
		...
		$q = $con->query("SELECT * FROM mdt_logs WHERE category = '{$type}' AND visible = 1 ORDER BY dateline DESC LIMIT 7");
		$logs = array();
	...
	function getAdminLogs()
	{
		...
		$q = $con->query("SELECT * FROM mdt_logs WHERE (category = 'Admin' OR category = 'Security' OR category = 'Session' OR category = 'Civilian Management' OR category = 'Calls') AND visible = 1 ORDER BY dateline DESC");
		$logs = array();
	...
	function getmdt_usersGroups($userid)
	{
		...
		foreach($groups as $group){
			$q = $con->query("SELECT * FROM mdt_usergroups WHERE id = '{$group}'");
			$a = mysqli_fetch_assoc($q);
	...
	function getUserGroups()
	{
		...
		$q = $con->query("SELECT * FROM mdt_usergroups");
	...
	function getCADReports()
	{
		...
		$q = $con->query("SELECT * FROM mdt_reports");
	...
	function getReportInfo($id)
	{
		...
		$q = $con->query("SELECT * FROM mdt_reports WHERE id = '{$id}'");
		$a = mysqli_fetch_assoc($q);
	...
	function updateUsergroup($admin,$groupid,$name,$perms)
	{
		...
		$q = $con->query("UPDATE mdt_usergroups SET name = '{$name}', perms = '{$perms}' WHERE id = '{$groupid}'");

		$q2 = $con->query("SELECT * FROM mdt_usergroups WHERE id = '{$groupid}'");
		$a2 = mysqli_fetch_assoc($q2);
	...
	function getPois()
	{
		...
		$q = $con->query("SELECT * from mdt_pois");
		$pois = array();
	...
	function getVois()
	{
		...
		$q = $con->query("SELECT * FROM mdt_vois");
		$vois = array();
	...
	function createPoi($civ,$image,$reason,$notes)
	{
		...
		$q = $con->query("INSERT into mdt_pois VALUES(NULL,'{$civ}','{$image}','{$reason}','{$notes}')");
	...
	function createVoi($vehicle,$image,$reason,$notes)
	{
		...
		$q = $con->query("INSERT INTO mdt_vois VALUES(NULL,'{$vehicle}','{$image}','{$reason}','{$notes}')");
	...
	function getPoiInfo($poi)
	{
		...
		$q = $con->query("SELECT * from mdt_pois WHERE id = '{$poi}'");
	...
	function getVoiInfo($voi)
	{
		...
		$q = $con->query("SELECT * FROM mdt_vois WHERE id = '{$voi}'");
	...
	function updatePoi($id,$image,$reason,$notes)
	{
		...
		$q = $con->query("UPDATE mdt_pois SET image = '{$image}', reason = '{$reason}', notes = '{$notes}' WHERE id = '{$id}'");
	...
	function updateVoi($id,$image,$reason,$notes)
	{
		...
		$q = $con->query("UPDATE mdt_vois SET image = '{$image}', reason = '{$reason}', notes = '{$notes}' WHERE id = '{$id}'");
	...
	function clearPoi($id)
	{
		...
		$q = $con->query("DELETE from mdt_pois WHERE id = '{$id}'");
	...
	function clearVoi($id)
	{
		...
		$q = $con->query("DELETE FROM mdt_vois WHERE id = '{$id}'");
	...
	function getCadsForUser($userid)
	{
		...
		$q = $con->query("SELECT * FROM mdt_reports WHERE user = '{$userid}'");
		$reports = array();
